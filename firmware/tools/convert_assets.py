...
# Full-screen backgrounds: <output name> : [candidate source names, best first]
# PNG page exports are listed BEFORE the GIFs: a GIF is limited to a 256-colour
# palette, which is why home1/home2 looked washed out next to the PNG artwork.
BACKGROUNDS = {
    "splash":  ["splash page", "splash"],
    "home1":   ["home (page 1)", "home page 1", "home1", "home"],
    "home2":   ["home (page2)", "home (page 2)", "home page 2", "home2"],
    "ai":      ["ai response page", "ai", "chat"],
    "music":   ["music page", "music"],
    "keypad":  ["keypad page", "keypad", "dial"],
    "calling": ["calling page", "calling", "call"],
}
...
# Prefer a lossless, full-colour source when several files share a stem.
EXT_RANK = {".png": 0, ".bmp": 1, ".webp": 2, ".jpg": 3, ".jpeg": 3, ".gif": 4}


def find_source(root: Path, stem: str) -> Path | None:
    """Case-insensitive, extension-agnostic search anywhere under `root`."""
    target = stem.lower()
    hits = [p for p in sorted(root.rglob("*"))
            if p.is_file() and p.suffix.lower() in EXTS and p.stem.lower() == target]
    if not hits:
        return None
    return min(hits, key=lambda p: EXT_RANK.get(p.suffix.lower(), 9))
