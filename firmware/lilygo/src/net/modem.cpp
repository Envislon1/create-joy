...
static void handleLine(const String& line) {
  if (line.startsWith("RING")) {
    ...
    if (!in_call) {
      in_call = true;
      link_bus::sendCallIncoming(from.c_str());
      ui::showIncomingCall(from.c_str());   // TFT jumps to the Calling page
    }
    ...
    if (modemSerial.available()) {
      ...
      ui::smsAddIncoming(from.c_str(), body.c_str());   // land it in the inbox
    }
  }
}
