...
static void onPi(const JsonDocument& doc) {
  ...
  if (!strcmp(t, "state")) {
    ...
    app_state::clock_hhmm = (const char*)(doc["hhmm"] | "");
    app_state::notifyChanged();
  } else if (!strcmp(t, "power")) {
    app_state::battery  = doc["battery"]  | app_state::battery;
    app_state::charging = doc["charging"] | false;
    app_state::notifyChanged();
  } else if (!strcmp(t, "net_status")) {
    app_state::online = doc["online"] | false;
    app_state::notifyChanged();
  } else if (!strcmp(t, "sms")) {
    // Pi-relayed SMS (web app -> Pi -> TFT). Modem-received SMS arrives via
    // net/modem.cpp on the LilyGo build.
    ui::smsAddIncoming(doc["from"] | "unknown", doc["text"] | "");
  } else if (!strcmp(t, "contact")) {
    ui::addContact(doc["name"] | "", doc["number"] | "");
  } else if (!strcmp(t, "alarm")) {
    const char* label = doc["label"] | "Medication time";
    ui::toast(label);
