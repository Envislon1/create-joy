				 ...
				 int *is_single_link)
{
	 ...
	 */
	ret = seeed_of_get_dai_name(node, &dlc->dai_name);
	if (ret < 0)
		return ret;
...
static int asoc_simple_init_dai_link_params(struct snd_soc_pcm_runtime *rtd)
{
	...
	seeed_dai_link_set_params(dai_link, params);
					...
					bool is_top_level_node)
{
	...
	ret = seeed_parse_daifmt(dev, node, codec,
...
static SEEED_PLATFORM_REMOVE_RET seeed_voice_card_remove(struct platform_device *pdev)
{
	...
	seeed_platform_remove_return(0);
}
