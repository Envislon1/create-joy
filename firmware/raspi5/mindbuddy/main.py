...
class MindBuddy:
    ...
    def __init__(self, debug=False):
        ...
        self.music = MusicPlayer(
            CFG.alsa_out, CFG.spotify_username, CFG.spotify_password,
            CFG.spotify_client_id, CFG.spotify_client_secret,
            CFG.spotify_refresh_token, CFG.spotify_device_name,
            on_state=lambda st: self.link.send(st) if self.link else None,
    ...
    def run(self):
        if CFG.ui_enabled:
            self._start_ui()
        if self.music: self.music.start()
        ...
        self._greet_user()
        if self.ui:
            # pygame must own the main thread
            self.ui.run()
            self.stop.set()
        else:
    ...
    def _start_ui(self):
        """Attach the Pi 5 touchscreen. Every Pi->screen message is fanned out
        to both the LilyGo link and the local UI; UI taps re-enter the same
        `_on_lg_msg` handler, so the protocol stays single-sourced."""
        try:
            from .ui import TouchUI
        except Exception as e:
            log.warning("touch UI unavailable (%s) - running headless", e); return
        assets = str(Path(CFG.ui_assets).expanduser())
        size = (CFG.ui_width, CFG.ui_height) if CFG.ui_width and CFG.ui_height else None
        ui = TouchUI(assets, self._on_lg_msg, fullscreen=CFG.ui_fullscreen,
                     size=size, rotate=CFG.ui_rotate)
        ui.state.user_name = CFG.user_name
        ui.state.mode = self.mode
        ui.state.language = self.language
        ui.state.pipeline = self.pipeline_pref
        ui.state.volume = CFG.default_volume
        self.ui = ui

        link = self.link
        outer = self
        class _FanOut:
            def start(self): link.start()
            def stop(self):  link.stop()
            def send(self, msg: dict):
                try: link.send(msg)
                except Exception as e: log.debug("link send: %s", e)
                try: outer.ui.on_message(msg)
                except Exception: log.exception("ui message failed")
        self.link = _FanOut()

    def _status_loop(self):
        """Push clock + battery to the TFT status bar. The Pi is the clock
        master (RTC/NTP) — the ESP32 never guesses the time."""
        last_hhmm = None
        last_power = None
        while not self.stop.is_set():
            ...
            if hhmm != last_hhmm:
                last_hhmm = hhmm
                self.link.send({"type": "time", "hhmm": hhmm})
            power = self._read_power()
            if power and power != last_power:
                last_power = power
                self.link.send({"type": "power", **power})
            time.sleep(5)

    @staticmethod
    def _read_power() -> dict | None:
        """Battery percent + charging flag.

        Reads the standard Linux power-supply sysfs nodes exposed by a UPS
        HAT (PiSugar, Waveshare UPS, X728, ...). Returns None on a Pi with no
        battery, in which case the TFT keeps its last known reading.
        """
        base = Path("/sys/class/power_supply")
        if not base.is_dir(): return None
        for supply in sorted(base.iterdir()):
            try:
                cap = (supply / "capacity").read_text().strip()
                status = (supply / "status").read_text().strip().lower() if (supply / "status").exists() else ""
            except Exception:
                continue
            if not cap.isdigit(): continue
            return {"battery": max(0, min(100, int(cap))),
    ...
    def _on_lg_msg(self, m: dict):
        ...
        elif t == "music_cmd":       self._on_music_cmd(m)
        ...
        elif t == "med_delete":      self._on_med_delete(int(m.get("index", -1)))

    def _on_med_set(self, m: dict):
        """Reminder time edited directly on the TFT -> update cache + Supabase."""
        try:
            idx  = int(m.get("index", -1))
            item = {"hour": int(m.get("hour", 8)), "minute": int(m.get("minute", 0)),
                    "enabled": bool(m.get("enabled", True)),
                    "label": str(m.get("label") or "Reminder")}
        except Exception as e:
            log.warning("bad med_set: %s", e); return
        meds = list(getattr(self, "_meds_cache", None) or [])
        if 0 <= idx < len(meds): meds[idx] = item
        else:                    meds.append(item)
        self._meds_cache = meds
        self._med_fired.clear()
        log.info("med set from TFT: %02d:%02d %s", item["hour"], item["minute"], item["label"])
        if self.sync:
            try: self.sync.push({"med": item, "meds": meds})
            except Exception as e: log.warning("med push failed: %s", e)

    def _on_med_delete(self, idx: int):
        meds = list(getattr(self, "_meds_cache", None) or [])
        if not (0 <= idx < len(meds)): return
        meds.pop(idx)
        self._meds_cache = meds
        self._med_fired.clear()
        if self.sync:
            try: self.sync.push({"meds": meds})
            except Exception as e: log.warning("med push failed: %s", e)

    def _on_music_cmd(self, m: dict):
        if not self.music: return
        self.music.command(m.get("cmd", "toggle"), source=m.get("source"), uri=m.get("uri"))

    def _on_mood(self, mood: str):
        """Mood tapped on TFT Home 2 -> mirror to Supabase + let the AI react."""
        if not mood: return
        log.info("mood check-in: %s", mood)
        try: self.sync.push_mood(mood)
        except Exception as e: log.warning("mood push failed: %s", e)
        self.turn_q.put(
    ...
    def _on_supabase_state(self, s: dict):
        ...
        self._meds_cache = s.get("meds", []) or []
        self.link.send({"type": "meds", "items": self._meds_cache})
