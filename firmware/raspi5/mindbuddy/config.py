...
class Config:
    ...
    # LC Technology RPI_AC108 4-mic array. The X-Powers AC108 quad ADC is
    # driven by the `ac108` codec driver (I2C control @0x3b + I2S/TDM audio);
    # its ALSA card registers as `seeed4micvoicec`. Override via .env if your
    # driver build names the card `ac108`.
    alsa_in:  str = _s("ALSA_IN",  "default")
    # AC108 captures 4 TDM channels; they are downmixed to mono for STT.
    mic_channels: int = _i("MIC_CHANNELS", 4)
    ...
    # --- Pi 5 touchscreen UI (Pi is the master; LilyGo only does calls/SMS) ---
    ui_enabled: bool = _s("UI_ENABLED", "1") not in ("0", "false", "no")
    ui_assets: str = _s("UI_ASSETS", "./assets")
    ui_fullscreen: bool = _s("UI_FULLSCREEN", "1") not in ("0", "false", "no")
    ui_rotate: int = _i("UI_ROTATE", 0)
    ui_width: int = _i("UI_WIDTH", 0)
    ui_height: int = _i("UI_HEIGHT", 0)
    user_name: str = _s("USER_NAME", "there")

    # --- music ---
    spotify_username: str = _s("SPOTIFY_USERNAME")
    spotify_password: str = _s("SPOTIFY_PASSWORD")
    spotify_client_id: str = _s("SPOTIFY_CLIENT_ID")
    spotify_client_secret: str = _s("SPOTIFY_CLIENT_SECRET")
    spotify_refresh_token: str = _s("SPOTIFY_REFRESH_TOKEN")
    spotify_device_name: str = _s("SPOTIFY_DEVICE_NAME", "MindBuddy")
