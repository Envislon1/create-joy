...
class AudioIO:
    ...
    def start_capture(self):
        try:
            self._open_stream(self.in_channels)
        except Exception as e:
            # Some ac108 driver builds only expose 2 channels until the
            # overlay is loaded; fall back so the box still hears the user.
            log.warning("ac108 %d-ch capture failed (%s) - retrying at 2ch",
                        self.in_channels, e)
            self._open_stream(2)
        log.info("capture started on %s (%dch)", self.in_device, self.in_channels)

    def _open_stream(self, channels: int):
        self._stream = sd.InputStream(
            device=self.in_device, channels=channels, samplerate=self.sr,
