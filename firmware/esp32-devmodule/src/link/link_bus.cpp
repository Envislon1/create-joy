...
void sendMoodSet(const char* mood) { JsonDocument d; d["type"]="mood_set"; d["mood"]=mood; send(d); }
void sendSmsSent(const char* to, const char* text) {
  JsonDocument d; d["type"]="sms_sent"; d["to"]=to; d["text"]=text; send(d);
}
void sendContactSaved(const char* name, const char* number) {
  JsonDocument d; d["type"]="contact_saved"; d["name"]=name; d["number"]=number; send(d);
}
void sendMedSet(int index, int hour, int minute, bool enabled, const char* label) {
  JsonDocument d; d["type"]="med_set"; d["index"]=index; d["hour"]=hour;
  d["minute"]=minute; d["enabled"]=enabled; d["label"]=label; send(d);
}
void sendMedDelete(int index) {
  JsonDocument d; d["type"]="med_delete"; d["index"]=index; send(d);
}
} // namespace
