...
| `mood_set` | `{mood:"..."}` | Mood check-in tapped on Home 2 |
| `sms_sent` | `{to:"...", text:"..."}` | Outgoing SMS, for the Pi/Supabase log |
| `contact_saved` | `{name:"...", number:"..."}` | New contact created on the keypad page |
...
| `power` | `{battery:0-100, charging:bool}` | Drives the battery icon in the TFT status bar |
| `time` | `{hhmm:"14:03"}` | Pi is the clock master (RTC + NTP); TFT never guesses |
| `net_status` | `{online:bool, rssi:int, mode:"wifi"\|"lte"}` | Drives the net / nonet icon |
| `sms` | `{from:"+234...", text:"..."}` | Push an SMS into the TFT inbox (web app -> Pi -> TFT) |
| `contact` | `{name:"Ada", number:"+234..."}` | Add/refresh a phone-book entry shown on the keypad page |
| `pong` | `{}` | reply |
...
### `med_set` (board -> Pi)
Reminder time edited on the TFT Medication page.
```json
{ "type": "med_set", "index": 0, "hour": 8, "minute": 30, "enabled": true, "label": "Reminder" }
```
`index` beyond the current list length appends a new reminder.

### `med_delete` (board -> Pi)
```json
{ "type": "med_delete", "index": 0 }
```
